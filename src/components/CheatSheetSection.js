      ...
      <div className={`cheatsheet-card ${unlocked ? 'unlocked' : 'locked'} ${completed ? 'completed' : ''}`}>
        <div className="card-header">
          <div className="letter-visual">
            <span className={`greek-letter-display ${unlocked ? '' : 'locked'}`}>
              {letter.greekLetter}
            </span>
          ...
          <div className="letter-info">
            <h3>{letter.name}</h3>
            <p className="pronunciation">/{letter.englishSound}/</p>
            {unlocked && (
            ...
            {letter.commonWords.length > 0 && (
              <div className="greek-section">
                <div className="greek-words">
                  <span className="greek-label">Ελληνικά:</span>
    ...
    return (
      <div className={`cheatsheet-list-item ${completed ? 'completed' : ''}`}>
        <div className="list-item-main">
          <span className="list-letter">{letter.greekLetter}</span>
          <span className="list-name">{letter.name}</span>
          <span className="list-sound">/{letter.englishSound}/</span>
  ...
  return (
    <div className="cheatsheet-section">
      <div className="section-header">
        <h2>Greek Alphabet Cheat Sheet</h2>
        <p>Comprehensive quick reference for all Greek letters, pronunciations, and visual aids.</p>
        ...
        <div className="cheatsheet-controls">
          ...
          <div className="filter-buttons">
            ...
            >
              Available ({greekAlphabet.filter(l => isLetterUnlocked(l.id)).length})
            </button>
            <button
              className={`filter-btn ${filter === 'completed' ? 'active' : ''}`}
              onClick={() => setFilter('completed')}
            >
              Completed ({greekAlphabet.filter(l => isLetterCompleted(l.id)).length})
            </button>
            <button
              className={`filter-btn ${filter === 'vowel' ? 'active' : ''}`}
              onClick={() => setFilter('vowel')}
            >
              Vowels ({greekAlphabet.filter(l => l.category === 'vowel' && isLetterUnlocked(l.id)).length})
            </button>
            <button
              className={`filter-btn ${filter === 'consonant' ? 'active' : ''}`}
              onClick={() => setFilter('consonant')}
            >
              Consonants ({greekAlphabet.filter(l => l.category === 'consonant' && isLetterUnlocked(l.id)).length})
            </button>
          </div>
        </div>
      </div>
