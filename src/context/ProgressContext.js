...
export const ProgressProvider = ({ children }) => {
  ...
  const ACHIEVEMENTS = {
    ...
    alphabetExplorer: {
      ...
      icon: '🗺️',
      requirement: (progress) => progress.completedLetters.length >= 5,
      reward: 'Unlocks Level 2 letters'
    },
    scholar: {
      id: 'scholar',
      name: 'Scholar',
  ...
  const checkForUnlocks = (currentScores, currentlyUnlocked) => {
    ...
    const currentLevel = Math.max(...greekAlphabet
    ...
    const currentLevelLetters = greekAlphabet.filter(letter =>
    ...
    if (averageCurrentLevelScore > 70 && currentLevel < 4) {
      const nextLevelLetters = greekAlphabet.filter(letter => letter.difficulty === currentLevel + 1);
